import { ReactPage, View, animatePageForView } from '../components';
...
abstract class TrRouter {
	...
	constructor(routerConfig?: Partial<TrRouterConfig>) {
		...
		TrNavigationEvents.on('pageDidMount', (pageElement: HTMLElement) => {
			setTimeout(() => {
				animatePageForView(pageElement.parentElement);
			}, 10);
		});

		TrNavigationEvents.on('pageDidUpdate', (pageElement: HTMLElement) => {
			setTimeout(() => {
				animatePageForView(pageElement.parentElement);
			}, 10);
		});

		TrNavigationEvents.on('pageWillUnmount', (pageElement: HTMLElement) => {
